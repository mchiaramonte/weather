...
while var == 1 :
    ...
    placeText(d, 0, 20, str(data["tempf"]) + "F", fnt, inkyphat.BLACK) 
    placeText(d, 1, 20, str(data["humidity"]) + "%", fnt, inkyphat.BLACK)
    placeText(d, 2, 20, str(data["windspeedmph"]*C_KTS) + "KTS", fnt, inkyphat.BLACK)
    placeText(d, 3, 20, WIND_DIRECTIONS[iPosition], fnt, inkyphat.BLACK)
    placeText(d, 0, 80, str(data["hl"]["tempf"]["h"]) + "/" + str(data["hl"]["tempf"]["l"]), fnt, inkyphat.BLACK)
    placeText(d, 2, 80, str(data["windgustmph"]*C_KTS) + "KTS", fnt, inkyphat.BLACK)
    now = datetime.now()
